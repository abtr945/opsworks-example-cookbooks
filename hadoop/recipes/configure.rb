...
# Step 2: (Master only) Enable passwordless SSH login from Master Node to all Slave Nodes

if node[:opsworks][:instance][:hostname] == "master"

  log "start_2" do
    message "<AN_TRAN> STEP 2: (Master only) Enable passwordless SSH login from Master Node to all Slave Nodes started"
    level :info
  end
  ...
  log "complete_2" do
    message "<AN_TRAN> STEP 2: (Master only) Enable passwordless SSH login from Master Node to all Slave Nodes completed"
    level :info
  end
  ...
  log "not_master" do
    message "<AN_TRAN> STEP 2: (Master only) Enable passwordless SSH login from Master Node to all Slave Nodes - This is not a master node, do nothing"
    level :info
  end
